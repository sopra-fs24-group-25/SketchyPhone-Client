...
const Game = () => {
    ...
    useEffect(() => {
        if (prevTask.current !== gameSession.current.gameLoopStatus) {
            ...
            //     console.log(prevTask.current, gameSession.current.gameLoopStatus);
            //     incrementRound(user.current, gameObject);
            //     console.log("incrementing Roundcounter");
            // }
            if (gameSession.current.gameLoopStatus === GameLoopStatus.TEXTPROMPT && !isInitialPrompt) {
                fetchDrawing();
            }
    ...
    async function fetchPresentationElements(user: User, game: GameObject) {
        try {
            const requestHeader = { "Authorization": user.token, "X-User-ID": user.userId };
            const url = `/games/${game.gameId}/sequence`;
            const response = await api.get(url, { headers: requestHeader })
    ...
    async function fetchGameUpdate(user: User, game: GameObject) {
        try {
            ...
            if (fetchedGameUpdate) {
                ...
                console.log(gameSession.current);
            }
        }
        catch (error) {
        ...
        return (
            <BaseContainer>
                ...
                <DrawContainer
                    ...
                    user={user.current}
                    game={gameObject}
                    textPrompt={receivedTextPrompt.current === null ? "" : receivedTextPrompt.current} // Just for testing
                    timerDuration={20}
                    setNextTask={setCurrentTask}
